...
def launch_setup(context, *args, **kwargs):
    ...
    base_configuration_file_path = (
        f'{get_package_share_directory("pom_description")}/config/pom_{robot_model}.yaml'
    )

    controller_manager_configuration_file_path = (
        f'{get_package_share_directory("pom_bringup")}/config/controller_manager.yaml'
    )

    base_controller_configuration_file_path = (
        f'{get_package_share_directory("pom_bringup")}/config/mobile_base_controller.yaml'
    )
    ...
    return LaunchDescription(
        [
            ...
            common.declare_robot_urdf_description(
                common.generate_robot_urdf_description("pom_bringup")
            ),
            common.declare_robot_ros2_control_description(
                common.generate_robot_ros2_control_description("pom_bringup")
            ),
