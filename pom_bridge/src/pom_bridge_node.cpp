...
int main(int argc, char * argv[])
{
  ...
  if (override_ros1_master) {
    ...
    if (ip.empty()) {
      RCLCPP_FATAL(
        ros2_node_ptr->get_logger(), "No ethernet connection with Pom robot!, Pom bridge failed");
      return 1;
    }
  ...
  ros::init(remappings, "pom_bridge");
  ...
  // Start bridge
  PomBridge bridge(ros1_node_ptr, ros2_node_ptr);
  bridge.start();
