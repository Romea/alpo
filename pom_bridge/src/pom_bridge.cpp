...
#include "pom_bridge.hpp"

const char pom_joy_topic[] = "/joy";
const char pom_odom_topic[] = "/pom_driver/ackermann_controller/odom";
const char pom_cmd_steer_topic[] = "/auto/cmd_steer";
const char pom_joint_states_topic[] = "/pom_driver/joint_states";
const char pom_battery_state_topic[] = "/power_supply/battery_status";

const char pom_impl_front_cmd_topic[] = "/cylinder/front/interface/command";
const char pom_impl_rear_cmd_topic[] = "/cylinder/rear/interface/command";
...
//-----------------------------------------------------------------------------
PomBridge::PomBridge(Ros1NodePtr ros1_node_ptr, Ros2NodePtr ros2_node_ptr)
: ros1_node_ptr_(ros1_node_ptr), ros2_node_ptr_(ros2_node_ptr)
{
}

//-----------------------------------------------------------------------------
void PomBridge::ros2_cmd_steer_callback_(const Ros2AckermannMsg::SharedPtr ros2_msg)
{
...
//-----------------------------------------------------------------------------
void PomBridge::ros2_impl_cmd_callback_(
...
//-----------------------------------------------------------------------------
void PomBridge::ros1_joy_callback_(const Ros1JoyMsg::ConstPtr ros1_msg)
{
  Ros2JoyMsg ros2_msg;
...
//-----------------------------------------------------------------------------
void PomBridge::ros1_odometry_callback_(const Ros1OdomMsg::ConstPtr & ros1_msg)
{
...
//-----------------------------------------------------------------------------
void PomBridge::ros1_joint_states_callback_(const Ros1JointStatesMsg::ConstPtr & ros1_msg)
{
...
//-----------------------------------------------------------------------------
void PomBridge::ros1_battery_state_callback_(const Ros1BatteryStateMsg::ConstPtr & ros1_msg)
{
...
void PomBridge::start()
{
  ...
  RCLCPP_INFO_STREAM(
    ...
    "Create sub 2 -> 1: " << bridge_cmd_steer_topic << " -> " << pom_cmd_steer_topic);
  RCLCPP_INFO_STREAM(
    ros2_node_ptr_->get_logger(),
    "Create sub 2 -> 1: " << bridge_impl_front_cmd_topic << " -> " << pom_impl_front_cmd_topic);
  RCLCPP_INFO_STREAM(
    ros2_node_ptr_->get_logger(),
    "Create sub 2 -> 1: " << bridge_impl_rear_cmd_topic << " -> " << pom_impl_rear_cmd_topic);
  RCLCPP_INFO_STREAM(
    ros2_node_ptr_->get_logger(),
    "Create pub 2 <- 1: " << bridge_joy_topic << " -> " << pom_joy_topic);
  RCLCPP_INFO_STREAM(
    ros2_node_ptr_->get_logger(),
    "Create pub 2 <- 1: " << bridge_odom_topic << " -> " << pom_odom_topic);
  RCLCPP_INFO_STREAM(
    ros2_node_ptr_->get_logger(),
    "Create pub 2 <- 1: " << bridge_joint_states_topic << " -> " << pom_joint_states_topic);
  RCLCPP_INFO_STREAM(
    ros2_node_ptr_->get_logger(),
    "Create pub 2 <- 1: " << bridge_battery_state_topic << " -> " << pom_battery_state_topic);
}

//-----------------------------------------------------------------------------
void PomBridge::init_ros1_publisher_()
{
  ros1_cmd_steer_pub_ = ros1_node_ptr_->advertise<Ros1AckermannMsg>(pom_cmd_steer_topic, 1);
  ros1_impl_front_cmd_pub_ =
    ros1_node_ptr_->advertise<Ros1ImplCmdMsg>(pom_impl_front_cmd_topic, 1);
  ros1_impl_rear_cmd_pub_ = ros1_node_ptr_->advertise<Ros1ImplCmdMsg>(pom_impl_rear_cmd_topic, 1);
}

//-----------------------------------------------------------------------------
void PomBridge::init_ros2_publishers_()
{
...
//-----------------------------------------------------------------------------
void PomBridge::init_ros1_subscriptions_()
{
  ros1_battery_state_sub_ = ros1_node_ptr_->subscribe(
    pom_battery_state_topic, 10, &PomBridge::ros1_battery_state_callback_, this);
  ros1_joint_states_sub_ = ros1_node_ptr_->subscribe(
    pom_joint_states_topic, 10, &PomBridge::ros1_joint_states_callback_, this);
  ros1_odom_sub_ =
    ros1_node_ptr_->subscribe(pom_odom_topic, 10, &PomBridge::ros1_odometry_callback_, this);
  ros1_joy_sub_ =
    ros1_node_ptr_->subscribe(pom_joy_topic, 10, &PomBridge::ros1_joy_callback_, this);
}

//-----------------------------------------------------------------------------
void PomBridge::init_ros2_subcription_()
{
  rclcpp::SubscriptionOptions options;
  options.ignore_local_publications = true;

  auto callback = std::bind(&PomBridge::ros2_cmd_steer_callback_, this, std::placeholders::_1);
