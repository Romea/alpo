...
The **Pom Bridge** package provides a communication bridge between ROS1 and ROS2 to interface with **Pom** robots, which run on a low-level ROS1 interface. This bridge allows the transmission of data from the robot’s embedded joystick and controller (including wheel joint states and odometry information) and enables command transmission to the robot.
...
- **/alpo_driver/ackermann_controller/odom** (nav_msgs::Odometry)
  ...
  This topic is pusblished by Pom  embedded controller and provides a lot informations about robot kinematic and dead reckoning.

- **/alpo_driver/joint_states** (sensors_msgs::JointState)

  This topic is also published by Pom embedded controller an provides informations about wheel joints states
...
- **~vehicle_controller/odom** (nav_msgs::msg::Odometry)
  ...
  This topic is broadcasted by the bridge and provides odometry message computed by Pom controller 

- **~vehicle_controller/joint_states** (sensor_msgs::msg::JointState)

  This topic is broadcasted by the bridge and provides joint states computed by Pom controller 
